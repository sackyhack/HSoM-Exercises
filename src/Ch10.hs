...
fringeOrig :: Int -> Cluster -> [SNote]
fringeOrig 0 (Cluster note cls) = [note]
fringeOrig n (Cluster note cls) = concatMap  (fringeOrig (n - 1)) cls

fringe :: Int -> Cluster -> [SNote]
fringe n c = fringeAux n c []
    where fringeAux :: Int -> Cluster -> [SNote] -> [SNote]
          fringeAux 0 (Cluster note cls) acc = note : acc
          fringeAux n (Cluster note cls) acc = foldr (fringeAux (n - 1)) acc cls

fringeTest :: Int -> [SNote] -> Bool 
fringeTest n notes = fringe n (selfSim notes) == fringeOrig n (selfSim notes)
...
ttm3 = let l1 = instrument Flute tm3
           l2 = instrument AcousticBass $ transpose (negate 9) (retro tm3)
       in l1 :=: l2
